...
try {
    ...
    $stmt = $bdd->prepare('
        ...
        FROM affectations a
        LEFT JOIN traitementsrealises t ON a.id_affectation = t.id_affectation
    ...
    // En-tête
    $pdf->Ln(10);
    $pdf->Cell(0, 5, utf8_decode('Le ' . $data['date_traitement']), 0, 1);

    // Informations patient
$html = '<table align="center" border="">
<hr />
<tr style="line-height:1px;">
<td width="350" height="50">' . utf8_decode('PAT-'. $data['id_patient'] . ' : ' . $data['nom_patient']) . '</td>
</tr>
<tr style="line-height:1px;">
<td width="350" height="50">' . utf8_decode(adress($data['adresse'])) . ' | ' . $data['phone'] . '</td>
</tr>
<hr /> <br>
<tr>
...
$pdf->WriteHTML($html);
    ...
    addSection($pdf, 'TRAITEMENT :', $data['details']);
