...
$caissier=" ";
    ...
    $reponse2 = $bdd->prepare('SELECT * FROM paiements_fournisseurs WHERE id_paie=?');
    $reponse2 -> execute(array($_GET['paiement']));
    $donnees2 = $reponse2->fetch();

    $reponse1 = $bdd->prepare('SELECT * FROM fournisseur_produit WHERE id_fournisseur=?');
    $reponse1 -> execute(array($donnees2['id_fournisseur']));
    $donnees1 = $reponse1->fetch();
    // Title
    $pdf->Cell(0,5,utf8_decode('Date Paiement :'.' '.$donnees2['date_ajout'].''),0,1);
//$pdf->Ln(2);
$pdf->SetFont('courier','B',13);
$pdf->Cell(0,10,utf8_decode('REÇU DE PAIEMENT FOURNISSEUR N°'.$donnees2['id_paie'].''),0,0,'C');
$pdf->Ln(10);
    $pdf->SetFont('courier','',11);
$html='<table align="center" border="">
<hr />
<tr style="line-height:1px;">
<td width="350" height="50">FOURNISSEUR : '.utf8_decode(patient($donnees2['id_fournisseur'])).'</td>
</tr>';
$html.='<tr style="line-height:1px;">
<td width="350" height="50" >CONTACT : '.contact($donnees2['id_fournisseur']).'</td>
</tr>';
$html.='<tr style="line-height:1px;">
<td width="350" height="50" >TYPE : '.$donnees1['type_fournisseur'].'</td>
</tr>';
$html.='<tr style="line-height:1px;">
...
$pdf->Ln(7);
$pdf->Cell(0,60,utf8_decode($donnees1['responsable']),0,0,'L');
