<?php
    ...
    $req = $bdd->prepare('INSERT INTO remise_de_compte (id_client, date_remise, montant, type_remise, mode_paiement, reference, id_compte, notes) VALUES(?,?,?,?,?,?,?,?)');
    $req->execute(array($_POST['id_client'], $_POST['date_remise'], $_POST['montant'], $_POST['type_remise'], $_POST['mode_paiement'], $_POST['reference'], $_POST['id_compte'], $_POST['notes']));
	...
	while ($donnes = $req0->fetch())
	{
		...
		$montantcompte = $debit+$_POST['montant'];
		$req3 = $bdd->prepare('UPDATE comptes SET debit='.$montantcompte.' WHERE id_compte=? ');
		$req3->execute(array($_POST['id_compte']));
                    ...
                    <div class="col-md-12">
							<section class="card">
								...
								<div class="card-body">
                                    <?php
                                        ...
                                        echo '
                                            <div class="alert alert-danger">
                                                <li>Remise de compte non effectuée, merci de vérifier les informations saisies</li>.
                                            </div>
                                            ';}
                                    ...
                                    ?>
                                    <form class="form-horizontal" novalidate="novalidate" method="POST" action="addremiseaccountout.php?newinaccount" enctype="multipart/form-data">
                                    <input type="hidden" name="ajouter" value="1">
									<div class="row form-group pb-3">
										<div class="col-md-4">
											<div class="form-group">
												<label class="col-form-label" for="formGroupExampleInput">Emis par</label>
												<select class="form-control populate" name="id_client" required="">
													<option>-------- Choisir le client --------</option>'
													<?php 
                                                        $client = $bdd->prepare('SELECT * FROM clients WHERE status=?');
                                                        $client -> execute(array(1));
                                                        while ($clients = $client->fetch())
                                                        {
                                                            echo '<option value="'.$clients['id_client'].'">'.$clients['nom_client'].'</option>';
                                                        } 
                                                    ?>
                                                </select>
											</div>
										...
										<div class="col-md-2">
											<div class="form-group">
												<label class="col-form-label" for="formGroupExampleInput">Remise éffectuée sur</label>
