...
try {
	...
	error_log('[imprimer_borderau] SHOW COLUMNS employes => ' . $e->getMessage());
}
...
try {
	...
	error_log('[imprimer_borderau] profil_entreprise => ' . $e->getMessage());
	$dataProfil = [];
}
...
try {
	...
	error_log('[imprimer_borderau] load remise => ' . $e->getMessage());
	$r = null;
}
...
$pdf->Cell(55, 8, pdf_text('RMISIC-' . (string)$idRemise), 1, 0, 'L');
...
$pdf->SetFont('CenturyGothic', '', 11);
$pdf->MultiCell(0, 6, pdf_text((string)($r['notes'] ?? '')), 0, 'L');

$pdf->Ln(10);
$pdf->SetFont('CenturyGothic', 'B', 11);
$pdf->Cell(95, 6, pdf_text('Signature du remettant'), 0, 0, 'L');
$pdf->Cell(95, 6, pdf_text('Signature  comptable'), 0, 1, 'R');
$pdf->Ln(22);
