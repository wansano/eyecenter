...
$pdf->Cell(0, 8, pdf_text_compat('DEMANDE LOGISTIQUE N° ' . $idDemande), 0, 1, 'C');

$pdf->SetFont('CenturyGothic', '', 11);
$pdf->Ln(2);
$pdf->Cell(0, 6, pdf_text_compat('Demandeur : ' . ($demandeurPseudo !== '' ? $demandeurPseudo : ('#' . $demandeurId))), 0, 1);
$pdf->Cell(0, 6, pdf_text_compat('Validateur : ' . ($validateurPseudo !== '' ? $validateurPseudo : ($validateurId > 0 ? ('#' . $validateurId) : '-'))), 0, 1);
...
$etatTrait = (int)($d['etat_traitement'] ?? 0);
$etatRec = (int)($d['etat_reception'] ?? 0);
if ($etatClin === 1) {
...
if ($etatTrait === 1 || $traitePar > 0) {
	...
	$pdf->Cell(0, 6, pdf_text_compat('Traité le ' . ($dt !== '' ? $dt : '-') . ' par ' . $who . ($sig !== '' ? (' | Signature : ' . $sig) : '')), 0, 1);
}
if ($etatRec === 1) {
	$sig = trim((string)($d['signature_reception'] ?? ''));
	$dt = appec_fmt_dt((string)($d['date_reception'] ?? ''));
	$pdf->Cell(0, 6, pdf_text_compat('Réception le ' . ($dt !== '' ? $dt : '-') . ($sig !== '' ? (' | Signature : ' . $sig) : '')), 0, 1);
}
...
$pdf->Cell(0, 6, pdf_text_compat('Le demandeur'), 0, 1, 'R');

$pdf->Output('DEMANDE_LOGISTIQUE_' . $idDemande . '.pdf', 'I');
