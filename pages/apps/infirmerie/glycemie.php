...
try {
    ...
    if (isset($_POST['consulter']) && !empty($_POST['glycemie'])) {
        ...
        if ($existe == 0) {
            ...
            try {
                insertGlycemie($bdd,  $affectation, $_POST);
                updateAffectationPourChirurgie($bdd, $affectation);
        ...
        <div class="inner-wrapper">
            <section role="main" class="content-body">
                <header class="page-header">
                    <h2>Glycémie d'un patient à opérer</h2>
                </header>
                            ...
                            <?php
                                        if ($errors==4) {
                                            echo '
                                                ...
                                                <strong>Glycémie pour '.model($type).' de '.nom_patient($id_patient).' validé avec succès </strong> <br/> 
                                                <li>Les informations relatives au traitement ont été enregistrées avec succès dans l\'espace du patient. Il peut toujours le consulter dans son propre espace</li>
                                                </div>
                                                ';
                                                    }
                                            if ($existe==1) {
                                            echo '
                                                <div class="alert alert-danger">
                                                    <strong>Erreur de validation de la glycémie pour '.model($type).'  de '.nom_patient($id_patient).'</strong> <br/>  
                                                    <li>Cette '.model($type).' a déjà été approuvé ou veuillez vérifier les informations requises à fournir.</li>
                                                </div>
                                                ';}
                                    ?>
                                ...
                                <div class="row form-group pb-3">                                    
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label class="col-form-label" for="formGroupExampleInput">Valeur de la glycémie</label>
                                            <input type="text" name="glycemie" class="form-control" placeholder="Obligatoire" value="<?php echo getFormValue('glycemie'); ?>" required>
                                        </div>
                                    </div>
                                </div>
