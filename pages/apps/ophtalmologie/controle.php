...
<body>
    <section class="body">
                    ...
                    <section class="card">
                        <div class="card-body">
                            <?php
                                        if ($errors==4) {
                                            echo '
                                                ...
                                                <li>Les informations relatives au traitement ont été enregistrées avec succès dans l\'espace du patient. Il peut toujours le consulter dans son propre espace ou <a href="#" onclick="openPrintControleModal(); return false;">imprimer les données</a></li>
                                                </div>
                                                ';
                                                    }
                                            if ($existe==1) {
                                            echo '
                                                <div class="alert alert-danger">
                                                    ...
                                                    <li>Cette '.model($type).' a déjà été approuvé ou veuillez vérifier les informations requises à fournir. Il est également possible <a href="#" onclick="openPrintControleModal(); return false;">d\'imprimer les données</a></li>
                                                </div>
                                                ';}
                                    ?>
                            ...
                            <footer class="card-footer text-end">
                                <button class="btn btn-primary" type="submit" name="ajouter" id="btnValiderControle" <?php echo ($errors==4 ? 'disabled' : ''); ?>>Valider le contrôle</button>
                            </footer>
                        </form>
                    </section>
                </div>
        ...
        <!-- Modal impression contrôle -->
        <div class="modal fade" id="printControleModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Impression du contrôle</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body" style="height: 75vh;">
                        <iframe id="printControleIframe" src="" style="width:100%;height:100%;border:0;"></iframe>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" onclick="printControleIframeNow()">Imprimer</button>
        ...
        <script>
            function openPrintControleModal() {
                try {
                    var modalEl = document.getElementById('printControleModal');
                    var iframe = document.getElementById('printControleIframe');
                    if (!modalEl || !iframe || !window.bootstrap) return;
                    iframe.src = 'imprimer_controle.php?affectation=<?php echo (int)$affectation; ?>&autoprint=0';
                    var instance = window.bootstrap.Modal.getInstance(modalEl) || new window.bootstrap.Modal(modalEl);
                    instance.show();
                } catch (e) {}
            }

            function printControleIframeNow() {
                try {
                    var iframe = document.getElementById('printControleIframe');
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                openPrintControleModal();
                var btn = document.getElementById('btnValiderControle');
