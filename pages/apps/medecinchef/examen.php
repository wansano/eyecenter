...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && !empty($_POST['diagnostic']) && !empty($_POST['resultat'])) {
        // Vérification si déjà traité
        $existe = checkTraitementExisteExamen($bdd, $affectation);
        ...
        if ($existe == 0) {
            ...
            try {
                ...
                insertExamen($bdd, $id_patient, $type, $affectation, $_POST);
                        ...
                        <div class="card-body">
                                        ...
                                        if ($errors==4) {
                                            ...
                                            echo '
                                                <div class="alert alert-danger">
                                                    ...
                                                    <li>Cette '.model($type).' a déjà été approuvé ou veuillez vérifier les informations requises à fournir. Il est également possible <a href="traitementdata.php?affectation='.$affectation.'" class="js-open-print" data-title="Impression données">d\'imprimer les données</a></li>
                                                </div>
                                                ';}
                                    ?>
                            ...
                            <form class="form-horizontal" novalidate="novalidate" method="POST"
                                ...
                                <div class="row form-group pb-3">                                    
                                    ...
                                    <div class="col-md-8">
                                        <div class="form-group">
                                            <label class="col-form-label" for="formGroupExampleInput">Resultat</label>
                                            <textarea name="resultat" class="form-control" rows="5" placeholder="Obligatoire" required><?php echo getFormValue('resultat'); ?></textarea>
                                        </div>
                                    </div>
                                </div>
            ...
            <script>
                window.addEventListener('load', function() {
                    if (typeof window.openPrintModal === 'function') {
                        window.openPrintModal('imprimer_examen.php?affectation=<?= (int)$affectation ?>', 'Impression examen');
                    }
                });
            </script>
        <?php endif; ?>
