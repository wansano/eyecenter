...
try {
    ...
    if (isset($_POST['consulter']) && !empty($_POST['diagnostic']) && !empty($_POST['conduite']) && !empty($_POST['prescription'])) {
        // Vérification si déjà traité
        $existe = checkTraitementExisteSoins($bdd, $affectation);
        ...
        if ($existe == 0) {
            ...
            try {
                ...
                insertSoins($bdd, $id_patient, $type, $affectation, $_POST);
                            ...
                            <?php
                                        if ($errors==4) {
                                            echo '
                                                ...
                                                <li>Les informations relatives au traitement ont été enregistrées avec succès dans l\'espace du patient. Il peut toujours le consulter dans son propre espace ou <a href="traitementdata.php?affectation='.$affectation.'" class="js-open-print" data-title="Impression données">imprimer les données</a></li>
                                                </div>
                                                ';
                                                    }
                                            if ($existe==1) {
                                            echo '
                                                <div class="alert alert-danger">
                                                    ...
                                                    <li>Ce '.model($type).' a déjà été approuvé ou veuillez vérifier les informations requises à fournir. Il est également possible <a href="traitementdata.php?affectation='.$affectation.'" class="js-open-print" data-title="Impression données">d\'imprimer les données</a></li>
                                                </div>
                                                ';}
                                    ?>
                                ...
                                <div class="row form-group pb-3">                                    
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            ...
                                            <textarea name="diagnostic" class="form-control" rows="5" placeholder="Obligatoire" required><?php echo getFormValue('diagnostic'); ?></textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label class="col-form-label" for="formGroupExampleInput">Conduite tenue</label>
                                            <textarea name="conduite" class="form-control" rows="5" placeholder="Obligatoire" required><?php echo getFormValue('conduite'); ?></textarea>
                                        </div>
                                    </div>
                                    <div class="col-md-4">
                                        <div class="form-group">
                                            <label class="col-form-label" for="formGroupExampleInput">Prescription</label>
                                            <textarea name="prescription" class="form-control" rows="5" placeholder="Facultatif" required><?php echo getFormValue('prescription'); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                                <div class="row form-group pb-3">
            ...
            <script>
                window.addEventListener('load', function() {
                    if (typeof window.openPrintModal === 'function') {
                        window.openPrintModal('imprimer_soins.php?affectation=<?= (int)$affectation ?>', 'Impression soins');
                    }
                });
            </script>
        <?php endif; ?>
